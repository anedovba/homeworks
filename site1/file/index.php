...
if (requestIsPost()) {
    if (isset($_FILES['attachment'])) {
        ...
        foreach ($files as &$file) {
            ...
            redirect("/?msg=error wrong type");
                continue;
            }

            if(!fileIsLessThan($file,30000000000)){
                redirect("/?msg=error size is too big");
                continue;
            }
...
//            }
            ...
            file_put_contents('image.txt',serialize([$album, $filename]).PHP_EOL,FILE_APPEND);
        }
        redirect('/?msg=files were uploaded');
    }
}
...
if (get('action') == 'delete') {
    if (!file_exists(get('file'))) {
        ...
        foreach ($images as &$image){
            ...
            {
                file_put_contents('image.txt',serialize($image).PHP_EOL,FILE_APPEND);
            }
        }

    }
    redirect('/?msg=files were deleted');
}
...
<form 	action="" method='post' enctype="multipart/form-data">
    ...
    <br>
    <input type="text" name="album">Укажите название альбома <br>
    <br>
